...
# Pygame için gerekli olan ayarlamalar
...
# Defining Objects as Wall, Food, Ghost
        ...
        if delay <= 0:
            if is_moving:
                ...
                if len(_ghost_new_position) > 0:
                    for idx in range(len(_ghost)):
                        ...
                        if timer >= SIZE_WALL:
                            ...
                            _map[new_row_Gho][new_col_Gho] = GHOST
                ...
                [row, col] = PacMan.getRC()
                 ...
                 # algoritma, new_PacMan_Pos değişkeninin yeni konumunu [yeni_satır, yeni_sütun] biçiminde döndürür
                 # Örnek: new_PacMan_Pos = [4, 5]

                search = SearchAgent(_map, _food_Position, row, col, N, M)
                if len(_food_Position) > 0:
                    new_PacMan_Pos = search.execute(ALGORITHMS=LEVEL_TO_ALGORITHM["LEVEL2"], visited=_visited)
                    _visited[row][col] += 1
